# -*- coding: utf-8 -*-
"""Laplacian_Spectral_K_means_Q2_iii_C.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1JEDf16OayqXf_bbAxVIsQtUxh7yQjtE8
...
def get_cluster_assignment(K,num_clusters,norm):
  ...
  return  cluster_assignment,error
...
def get_adjacency_matrix(dist_mat,radius):
  return (dist_mat>radius)*1 ##Setting the cutoff for distance to be considered as edge

def plot_clusters_Using_Laplacian_Spectral_Kmeans(X_dataset): 
  for d in range(5,15,2):
    D_mat=distance_matrix(X_dataset)
    Ad_mat=get_adjacency_matrix(D_mat,d)
    Laplacian = get_Laplacian(Ad_mat*1)
    cluster_assignment,error=get_cluster_assignment(Laplacian,4,"l2")


    plt.figure(d)
    sns.scatterplot(x=X_dataset[:,0],y=X_dataset[:,1],hue=cluster_assignment,palette="deep")
    plt.axis('on')
    plt.title("Spectral K-Means Clustering by using Laplacian with radius=" +str(d))
    plt.show()

plot_clusters_Using_Laplacian_Spectral_Kmeans(X_dataset)